...
-- Backfill existing rows by resolving localized service names to their canonical French name.
WITH alias AS (
  SELECT * FROM (VALUES
    ...
    ('Refill','Retouche'),
    ('Touch-up','Retouche')
  ) AS t(alias_name, canonical_name)
)
UPDATE public.bookings b
SET duration_min = s.duration_min
FROM alias a
JOIN public.services s ON s.name = a.canonical_name
WHERE b.duration_min IS NULL AND b.service = a.alias_name;
